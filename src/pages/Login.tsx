...
import { Package, Mail, Lock, Eye, EyeOff, Star, Shield, Zap } from 'lucide-react';
...
export default function Login() {
  ...
  return (
    <div className="min-h-screen flex relative overflow-hidden bg-slate-50 dark:bg-slate-950">
      ...
      <FloatingOrb className="bg-amber-500 top-1/2 left-1/3" delay="4s" size="w-48 h-48" />
        ...
        <div className="hidden lg:flex w-1/2 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 dark:from-black dark:via-slate-950 dark:to-black items-center justify-center p-12 relative overflow-hidden">
          ...
          <div className="relative text-center max-w-md">
            ...
            <div className="mt-10 space-y-4">
              {[
                { icon: Zap, text: t('feature.shipping') },
                { icon: Shield, text: t('feature.payment') },
                { icon: Star, text: t('feature.support_desc') },
              ].map(({ icon: Icon, text }) => (
            ...
            <div className="mt-12 pt-8 border-t border-white/10">
              <p className="text-white/40 text-sm">{t('login.noaccount')}</p>
              <Link to="/register" className="inline-flex items-center mt-2 text-accent-400 hover:text-accent-300 font-medium transition-colors">
                {t('login.create')}
              </Link>
            </div>
          </div>
        ...
        <div className="w-full lg:w-1/2 flex items-center justify-center p-4 sm:p-8 lg:p-12">
          <div className="w-full max-w-md">
            ...
            <div className="lg:hidden text-center mb-8">
              ...
              <p className="text-slate-500 dark:text-slate-400 mt-1">{t('nav.signin')}</p>
            </div>
            ...
            <div className="bg-white dark:bg-slate-800/80 backdrop-blur-xl rounded-3xl border border-slate-200 dark:border-slate-700/50 p-8 shadow-2xl shadow-slate-200/50 dark:shadow-black/20">
              <form onSubmit={handleSubmit} className="space-y-5">
                <div className="space-y-1">
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300">{t('login.email')}</label>
                ...
                <div className="space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" className="text-xs text-accent-600 hover:text-accent-500 transition-colors font-medium">{t('login.forgot')}</button>
                  </div>
                  <div className="relative group">
                    <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-accent-500/20 to-amber-500/20 opacity-0 group-focus-within:opacity-100 transition-opacity blur-xl" />
                ...
                <button type="submit" disabled={loading}
                  ...
                  <span className="relative">{loading ? t('login.loading') : t('login.button')}</span>
                </button>
              </form>
              ...
              <div className="mt-6">
                <div className="relative">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-white dark:bg-slate-800 px-3 text-slate-500">{t('login.or')}</span>
                  </div>
                </div>
              ...
              <div className="mt-6 text-center lg:hidden">
                <p className="text-sm text-slate-500 dark:text-slate-400">
                  {t('login.noaccount')}{' '}
                  <Link to="/register" className="text-slate-900 dark:text-white hover:text-accent-500 dark:hover:text-accent-400 font-medium transition-colors">{t('login.create')}</Link>
                </p>
              </div>

              <div className="mt-4 p-4 bg-slate-100 dark:bg-slate-700/50 rounded-xl border border-slate-200 dark:border-slate-600">
                <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">{t('login.demo')}</p>
                <div className="space-y-1 text-xs text-slate-500 dark:text-slate-400">
                  <p><span className="text-slate-900 dark:text-slate-100 font-medium">{t('login.demo_admin')}:</span> admin@example.com / admin123</p>
                  <p><span className="text-slate-900 dark:text-slate-100 font-medium">{t('login.demo_user')}:</span> ahmed@example.com / user123</p>
                </div>
              </div>
            </div>
          </div>
